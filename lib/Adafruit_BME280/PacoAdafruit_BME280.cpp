...
/*
  Modificaciones sobre la original:
    -1. He añadido una función nueva: readHumidityTemperature(float*, float*)
        Para leer ambos parámetros en un ciclo de bus i2c

    -2. El i2c se inicializa a 400KHz
    -3. Las funciones de la librería no inicializan el i2c
...
/*!
 *  @brief  Returns the humidity from the sensor. Modifica temperatura y humedad  
 *  cuyas direcciones se pasan como parámetros los valores de temperatura y humedad
 *  @returns the humidity value read from the device
 */

float Adafruit_BME280::readHumidityTemperature(float *temperatura,float* humedad){
  *temperatura = readTemperature(); // must be done first to get t_fine
  int32_t adc_H = read16(BME280_REGISTER_HUMIDDATA);
  if (adc_H == 0x8000) // value in case humidity measurement was disabled
    return NAN;
